...
class GRUPrior(nn.Module):
    """ A GRU-based prior model for predicting the next state. """
    def __init__(self, n_embd):
        super().__init__()
        # The GRUCell takes the input and the previous hidden state
        self.gru_cell = nn.GRUCell(n_embd, n_embd)
        self.ln = nn.LayerNorm(n_embd)

    def forward(self, x, h_past):
        """
        x: The input for the current step (e.g., h_mha_past)
        h_past: The previous hidden state of this GRU cell
        """
        # Compute the next hidden state
        h_next = self.gru_cell(x, h_past)
        # We can add a layer norm for stability
...
class DynamicBlock(nn.Module):
    ...
    def __init__(self, n_embd, n_head, prior_model, k_surprise_threshold=2.0):
        super().__init__()
        head_size = n_embd // n_head
        self.sa = MultiHeadAttention(n_head, head_size) 
        self.ffwd = FeedFoward(n_embd)
